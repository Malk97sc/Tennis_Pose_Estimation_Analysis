...
def main():
    video_name = "us_open" 
    video_path = RAW_DATA_DIR / (video_name + ".mp4")
    output_path = Path(DATA_DIR) / "results" / "video"
    output_path.mkdir(parents=True, exist_ok=True)
    
    #--------models-----------
    yolo_model = "yolo11x"
    player_model_path = MODELS_DIR / "yolo" / (yolo_model + ".pt")
    ...
    player_dt = player_track.detect_player(video, read_stub = False, stub_path = stub_path / "player_detection_y11.pkl") #player detection
    
    #--------ball-------------
    ball_track = BallTracking(ball_model_path, fps, confidence = 0.25) #ball tracking instance
    ball_dt = ball_track.detect_ball(video, read_from_stub = False, stub_path = stub_path / "ball_detection_y11.pkl") #ball detection
    ...
    player_pose_dt = pose_estimator.estimate_pose(video, player_dt, read_stub = False, stub_path = stub_path / "pose_estimation.pkl")
    ...
    #-------draw boxes----------
    out_video = player_track.draw_boxes(video, player_dt, hull_kp)  #draw player bbox
    #out_video = ball_track.draw_boxes(out_video, ball_dt) #draw ball bbox
    out_video = court_line.draw_keypoints_on_video(out_video, court_kp, hull_kp) #draw keypoints to show the court:  show_court = True
    out_video = pose_estimator.draw_pose(out_video, player_pose_dt) #draw pose estimation

    #------draw stats----------
    #out_video = draw_player_stats(out_video, player_stats_df)

    output_video_path = output_path / f"{video_name}_{yolo_model}_{interpolation_method}_pose.avi"
    ...
    black_video = player_track.draw_boxes(black_video, player_dt, hull_kp) #draw player bbox
    black_video = court_line.draw_keypoints_on_video(black_video, court_kp, hull_kp) #draw keypoints to show the court:  show_court = True
    black_video = pose_estimator.draw_pose(black_video, player_pose_dt) #draw pose estimation

    output_video_path = output_path / f"{video_name}_black_{yolo_model}_pose.avi"
